...
const ProductEdit = () => {
  ...
  const mapProductDataFromAPI = (apiData) => {
    ...
    try {
      return {
        ...
        category: { 
          id: apiData.category_id.toString(), 
  ...
  useEffect(() => {
    const loadProductData = async () => {
      ...
      try {
        ...
        // If we have product data from location state
        if (product) {
          const mappedProduct = mapProductDataFromAPI(product);
          setProductData(mappedProduct);
          
          // Set image URLs
          if (mappedProduct.images && mappedProduct.images.length > 0) {
            setImageUrls(mappedProduct.images.map(img => `${APIURL}/${img}`));
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (productData.sub_category?.id && !product) {
      loadSizes();
    }

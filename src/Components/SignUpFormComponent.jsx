  ...
  const handleRegistration = () => {
    ...
    if (!newErrors.firstName && !newErrors.lastName && !newErrors.email && !newErrors.password) {
      if(isChecked) {
        ...
        setResponse({ success: false, message: "Please accept the terms and conditions" });
      }
    }
  }
    ...
    <div className="bg-white rounded-3xl items-center shadow-lg overflow-hidden max-w-4xl w-full flex">
      ...
      <div className="w-full md:w-1/2 p-8 relative">
        ...
        <div className="space-y-6">
          ...
          {!response.success && <span className="text-red-500 text-sm">{response.message}</span>}
          ...
          {/* Submit Button */}
          <button onClick={handleRegistration} className="w-full bg-[#ffb700] text-black font-medium py-3 rounded-full hover:bg-yellow-500 transition-colors">
            Create Account
          </button>
